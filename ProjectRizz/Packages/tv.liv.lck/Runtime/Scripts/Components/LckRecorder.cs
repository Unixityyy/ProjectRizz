...
namespace Liv.Lck.Recorder
{
    internal class LckRecorder : ILckRecorder
    {
        ...
        private float CurrentRecordingDurationSeconds => Time.time - _recordingStartTime;
        ...
        public LckResult<TimeSpan> GetRecordingDuration()
        {
            ...
            return LckResult<TimeSpan>.NewSuccess(TimeSpan.FromSeconds(CurrentRecordingDurationSeconds));
        }
                        ...
                        LckMonoBehaviourMediator.Instance.EnqueueMainThreadAction(() =>
                        {
                            if (success)
                            {
                                ...
                                var recordingData = new RecordingData
                                {
                                    ...
                                    RecordingDuration = CurrentRecordingDurationSeconds
                                };
        ...
        private void SendRecordingStoppedTelemetry()
        {
            ...
            var actualFramerate = (CurrentRecordingDurationSeconds > 0 && encodedVideoFrameCount > 0) 
                ? (encodedVideoFrameCount / CurrentRecordingDurationSeconds) 
                : 0f;
            _recordingTelemetryContext.Add("recording.duration", CurrentRecordingDurationSeconds);
        ...
        private void OnCaptureError(CaptureErrorEvent captureErrorEvent)
        {
            ...
            _telemetryClient.SendErrorTelemetry(LckResult.NewError(LckError.RecordingError, errorMsg));
