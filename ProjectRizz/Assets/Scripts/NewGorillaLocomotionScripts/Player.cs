namespace GorillaLocomotion
{
    ...
    public class Player : MonoBehaviour
    {
        ...
        public bool leftHaptic = false;
        public bool rightHaptic = false;
        public bool disableMovement = false;
        public int JSMaterial = 0;
        ...
        public void TriggerHaptic(XRBaseController controller)
        {
            ...
            {
                controller.SendHapticImpulse(VibrateIntensity, VibrateDuration);
            }
        }
        ...
        private void OnAchievementUnlocked(Message msg) { if (msg.IsError) { Debug.LogError("Failed to unlock achievement: " + msg.GetError().Message); } else { Debug.Log("Achievement unlocked successfully!"); } }

        private void OnEntitlementCheckComplete(Message msg)
        {
            if (msg.IsError)
            {
                Debug.LogError("Oculus Entitlement Check Failed: " + msg.GetError().Message);
            }
            else
            {
                Debug.Log("Oculus Entitlement Check Succeeded");
            }
        }
        ...
        private void Update()
        {
            ...
            if (leftHandColliding)
            {
                if (!leftHaptic)
                {
                    TriggerHaptic(leftHandController);
                    // Debug.Log("triggering haptic for left controller");
                    leftHaptic = true;
                }
            } 
            else
            ...
            if (rightHandColliding)
            {
                if (!rightHaptic)
                {
                    TriggerHaptic(rightHandController);
                    // Debug.Log("triggering haptic for right controller");
                    rightHaptic = true;
                }
            }
            else
